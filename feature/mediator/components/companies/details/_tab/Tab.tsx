...
interface I_TabProps {
  ...
  active: string;
  updates?: number;
  onValueChange: (value: keyof typeof companyTabsItemEnums) => void;
}
...
const Tab = ({ items, active, updates, onValueChange }: I_TabProps) => {
  return (
    <>
            ...
            {items.map((item) => (
              <Typography
                ...
                className={cn(
                  ...
                  "pb-2 font-bold hover:border-violet-normal",
      ...
      <Desktop>
        <Card className="rounded-2xl rounded-b-none xl:px-8 xl:py-6 xl:pt-9">
